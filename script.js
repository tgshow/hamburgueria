...
checkoutBtn.addEventListener("click", function () {
  if (!isOpen) {
    toastify({
      text: "Ops, a GaragemBurguer está fechada!",
      duration: 3000,
      close: true,
      gravity: "top", 
      position: "right", 
  ...
  if (addressInput.value === "") {
    ...
    const cartItems = cart.map((item) => {
      return (
        `${item.name} Quantindade: (${item.quantity}) Preço: R$${item.price} |`
      )
    }).join("")

    const message = encodeURIComponent(cartItems)
    const phone = "+5599984193189"
...
const isOpen = checRestaurantOpen();
